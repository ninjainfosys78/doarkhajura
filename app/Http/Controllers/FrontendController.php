...
use App\Models\PedigreeCaste;
use App\Models\PedigreeCategory;
...
class FrontendController extends BaseController
{
    ...
    public function index(DocumentCategory $documentCategory)
    {
        ...
        $galleries = PhotoGallery::with('photos')->whereHas('photos')->latest()->get();
        $videoGalleries = VideoGallery::latest()->limit(4)->get();
        $farmerHelpfulls = FarmerHelpfull::latest()->Limit(4)->get();
        $researchUnits = ResearchUnit::with('researchUnits.researchUnit')
            ->whereNull('research_unit_id')
            ->orderBy('research_unit_id')
            ->get();
        $documents = Document::with('files')->where('status', 1)->inRandomOrder()->latest()->limit(2)->get();
        return view('frontend.index', compact('officeDetail', 'tickerFiles', 'sliders', 'categories', 'galleries', 'noticePopups', 'farmerHelpfulls', 'researchUnits', 'videoGalleries', 'documents'));
        if (config('default.subDivision')) {
            $subDivisions = SubDivision::latest()->get();
            $compact[] = 'subDivisions';
        }
        return view('frontend.index', compact($compact));
    }
    ...
    public function staticMenus($slug)
    {
        switch ($slug) {
            case 'farmerHelp':
                $farmerHelpfulls = FarmerHelpfull::latest()->get();
                return view('frontend.farmerHelpfullMaterial.index', compact('farmerHelpfulls'));
                break;
            case 'agronomy':
                $agronomies = Agronomy::latest()->get();
                return view('frontend.argonomy.index', compact('agronomies'));
                break;
            case 'contactUs':
    ...
    public function  argonomy(ResearchUnit $researchUnit)
    {
        $farmerHelpfulls = FarmerHelpfull::where('research_unit_id', $researchUnit->id)->latest()->limit(4)->get();
        $galleries = PhotoGallery::with('photos')->where('research_unit_id', $researchUnit->id)->latest()->get();
        $videoGalleries = VideoGallery::where('research_unit_id', $researchUnit->id)->latest()->limit(4)->get();
        $agronomy = Agronomy::with(['researchUnit', 'TechnicalOfficer', 'Scientist'])
            ->where('research_unit_id', $researchUnit->id)
            ->latest()
            ->get();
        $documents = Document::with('files')->where('research_unit_id', $researchUnit->id)->where('status', 1)->latest()->limit(4)->get();
        return view('frontend.argonomy.index', compact('galleries', 'agronomy', 'researchUnit', 'videoGalleries', 'farmerHelpfulls', 'documents'));
    }
    public function  employeeDetail(Employee $employee)
    {
        return view('frontend.employeeDetail.index', compact('employee'));
    }
    public function farmerUsefullMaterials()
    {
        $farmerHelpfulls = FarmerHelpfull::latest()->get();
        return view('frontend.farmerHelpfullMaterial.index', compact('farmerHelpfulls'));
    }

    public function farmerDetail(FarmerHelpfull $farmerHelpfull)
    {
        return view('frontend.farmerHelpfullMaterial.fileDetail', compact('farmerHelpfull'));
    }
}
