    ...
    <div class="card-style mb-30">
        <form action="{{route('admin.officeSetting.update',$officeSetting)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                {{-- <div class="col-md-4">
                    <div class="input-style-1">
                        ...
                        <select name="siemens_officer_id" id="siemens_officer_id" class="form-control">
                            <option value="">- - छान्नुहोस् - -</option>
                            @foreach($employees as $employee)
                                <option value="{{$employee->id}}" {{$officeSetting->siemens_officer_id==$employee->id ? 'selected' : ''}}>{{$employee->name}}</option>
                            @endforeach
                        </select>
                        @error('siemens_officer_id')
                        <p class="text-danger">{{$message}}</p>
                        @enderror
                    </div>
                </div> --}}
                ...
                <div class="col-md-4">
                    <div class="input-style-1">
                        <label for="distribution_officer_id">वितरण अधिकारी</label>
                        <select name="distribution_officer_id" id="distribution_officer_id" class="form-control">
                            <option value="">- - छान्नुहोस् - -</option>
                            @foreach($employees as $employee)
                                <option value="{{$employee->id}}" {{$officeSetting->distribution_officer_id==$employee->id ? 'selected' : ''}}>{{$employee->name}}</option>
                            @endforeach
                        </select>
                        @error('distribution_officer_id')
                        <p class="text-danger">{{$message}}</p>
                        @enderror
                    </div>
                </div>
