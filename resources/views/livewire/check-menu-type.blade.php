            ...
            <div class="input-style-1">
                ...
                <select name="model_id" id="model_id" wire:model="model_id" class="form-control">
                    ...
                    @foreach($menuTypes as $menuType)
                        <option value="{{$menuType->id}}" @if($menu_type=="category" && count($menuType->documentCategories)>0) disabled @elseif($menu_type=="PedigreeCaste") disabled @endif>
                            {{$menuType->title}}
                        </option>
                        ...
                        @if($menu_type=="PedigreeCaste")
                            @foreach($menuType->pedigreeCaste as $pedigreeCaste)
                                <option value="{{$pedigreeCaste->id}}">&nbsp; - - {{$pedigreeCaste->title}}</option>
                            @endforeach
                        @endif
                        @if($menu_type=="PedigreeDistribution")
                            @foreach($menuType->pedigreeCaste as $pedigreeCaste)
                                <option value="{{$pedigreeCaste->id}}">&nbsp; - - {{$pedigreeCaste->title}}</option>
                            @endforeach
                        @endif
